(() => {
    ...
    async function initFlashcard() {
        ...
        /* ========================================
           카드 중앙 문구 자동 맞춤
           원래 지정된 크기를 우선 사용하고,
           카드 내부 영역을 넘을 때만 축소합니다.
        ======================================== */

        let textFitFrame = 0;

        function fitMessageToFace(face) {
            const message =
                face.querySelector(".card-message");
            ...
            const verticalPadding =
                parseFloat(faceStyle.paddingTop) +
                parseFloat(faceStyle.paddingBottom);

            const eyebrow =
                face.querySelector(".card-eyebrow");

            const footer =
                face.querySelector(".card-footer");
        ...
        /* ========================================
           INDEPENDENT REFERENCE IMAGE CAROUSEL
           - Uses an optional cover image first, then numbered files (1.png, 2.png, ...).
           - Image movement is independent from flashcard movement.
           - Left/right arrow keys move only this image carousel.
        ======================================== */

        const lessonCardImage =
            document.getElementById("lessonCardImage");

        const mediaPagination =
            document.getElementById("mediaPagination");

        const mediaPrevButton =
            document.getElementById("mediaPrevButton");

        const mediaNextButton =
            document.getElementById("mediaNextButton");

        const referenceImageCount =
            Math.max(
                0,
                Number.parseInt(
                    document.body.dataset.imageCount || "0",
                    10
                ) || 0
            );

        const referenceCoverImage =
            String(
                document.body.dataset.imageCover || ""
            ).trim();

        const referenceSlideCount =
            referenceImageCount +
            (referenceCoverImage ? 1 : 0);

        const referenceImageExtension =
            String(
                document.body.dataset.imageExt || "png"
            ).replace(/^\./, "");

        let currentReferenceImageIndex = 0;

        function renderReferencePagination() {
            const hasMultipleImages =
                referenceSlideCount > 1;

            if (mediaPrevButton) {
                mediaPrevButton.hidden =
                    !hasMultipleImages;
            }

            if (mediaNextButton) {
                mediaNextButton.hidden =
                    !hasMultipleImages;
            }
            ...
            for (
                ...
                dot.setAttribute(
                    "aria-label",
                    `${index + 1} / ${referenceSlideCount}`
                );

                mediaPagination.append(dot);
            }
        }
        ...
        function renderReferenceImage() {
            ...
            if (referenceSlideCount === 0) {
                lessonCardImage.hidden = true;
                lessonCardImage.removeAttribute("src");
                lessonCardImage.alt = "";
            ...
            const folder =
                document.body.dataset.imageFolder ||
                "./images";

            const hasCover =
                Boolean(referenceCoverImage);
            ...
            const imageFileName =
                hasCover && currentReferenceImageIndex === 0
                    ? referenceCoverImage
                    : `${imageNumber}.${referenceImageExtension}`;

            const displayPosition =
                currentReferenceImageIndex + 1;

            lessonCardImage.onload = () => {
                lessonCardImage.hidden = false;
                lessonCardImage.alt =
                    `참고 이미지 ${displayPosition} / ${referenceSlideCount}`;
        ...
        function goToReferenceImage(direction) {
            if (referenceSlideCount <= 1) {
                return;
            }
        ...
        mediaPrevButton?.addEventListener(
            "click",
            event => {
                event.preventDefault();
                event.stopPropagation();
                goToReferenceImage(-1);
            }
        );

        mediaNextButton?.addEventListener(
            "click",
            event => {
                event.preventDefault();
                event.stopPropagation();
